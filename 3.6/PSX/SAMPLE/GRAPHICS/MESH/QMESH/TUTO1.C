...
SVECTOR map_vert[RECTV+1][RECTH+1];	/* position: 座標値*/
SVECTOR map_text[RECTV+1][RECTH+1];	/* texture position: テクスチャー座標*/
...
main()
{
	...
	*/
	...
	SetDispMask(1);		/* enable to display (0:inhibit, 1:enable) */
        ...
        /* generate position (x,y,z): X,Y,Zを生成する*/
        gen_vert(map_vert,RECTV,RECTH,EDGE);

	/* generate texture postion (u,v): テクスチャーアドレスを生成する*/
	gen_texture(map_text,RECTV,RECTH);

	/* make QMESH strcuture: QMESH構造体にする */
	mesh.lenv=RECTV+1;
	mesh.lenh=RECTH+1;
