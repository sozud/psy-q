...
#define OT_LENGTH  12		/* bit length of OT : オーダリングテーブルの解像度 */
...
GsCOORDINATE2 DWorld;	/* Coordinate for GsDOBJ2:オブジェクトごとの座標系 */

GsCOORDINATE2 DView;		/* Coordinate for View : 視点をぶら下げる座標系 */
...
{				/* initialize rotines : 初期化ルーチン群 */
	...
	model_init();		/* set up the modeling data :モデリングデータ読み込み */
...
{				/* set up the modeling data :
				   モデリングデータの読み込み */
	u_long *dop;
	GsDOBJ2 *objp;		/* the handler or modeling data : モデリングデータハンドラ */
	int     i;
