...
/* Ot Handler of upper screen : オーダリングテーブル１（画面上部）ハンドラ */
GsOT    Wot1[2];

/* Ot Handler of lower screen : オーダリングテーブル２（画面下部）ハンドラ */
GsOT    Wot2[2];
...
{				/* set the vieowpoint of upper screen :
	...
	/* set the view point from parameters (libgs caliculate World-Screen
	  Matrix) : 視点パラメータを群から視点を設定するワールドスクリーン
	  マトリックスを計算する */
	GsSetRefView2(&view);
}
