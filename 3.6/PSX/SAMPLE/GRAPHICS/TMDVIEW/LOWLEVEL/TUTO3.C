...
#define OBJECTMAX 100		/* Max Objects /*
	  ３Dのモデルは論理的なオブジェクトに 分けられるこの最大数を定義する */
...
obj_interactive()
{
	...
	if ((padd & PADl) > 0)
		DWorld.coord.t[2] += 100;
	/* Translate Z /* オブジェクトをX軸にそって動かす */
	if ((padd & PADLleft) > 0)
		DWorld.coord.t[0] += 10;
	/* Translate Z /* オブジェクトをX軸にそって動かす */
	if ((padd & PADLright) > 0)
		DWorld.coord.t[0] -= 10;
	/* Translate Z /* オブジェクトをY軸にそって動かす */
	if ((padd & PADLdown) > 0)
		DWorld.coord.t[1] += 10;
	/* Translate Z /* オブジェクトをY軸にそって動かす */
	if ((padd & PADLup) > 0)
...
init_all()
{
	...
	padd = 0;		/* Controller value initialize /* コントローラ値初期化 */

	GsInitGraph(640, 480, GsINTER | GsOFSGPU, 1, 0);
	/* resolution set , interlace mode /* 解像度設定（インターレースモード） */
...
model_init()
{
	...
	dop++;			/* Adjusting for GsLinkObject4 /*
				 GsLinkObject4でリンクするためにTMDのオブジェクト
				 の先頭にもってくる */
