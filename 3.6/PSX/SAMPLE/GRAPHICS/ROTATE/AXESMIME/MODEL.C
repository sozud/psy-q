...
/* TMD data addresses :  TMD データの置かれるアドレス */
...
/* packet buffer area for GsSort*() : GsSort*() が使うパケットバッファ領域 */
...
/* number of parent part : 親パーツ番号 */
static int parent_coord[/* PARTNUM */]={
    -1, 0, 1, 0, 3, 0, 5, 0, 7, 0
};

/* using TMD number: このパーツはどの TMD を使うか */
...
void model_hand_init(void);
void model_coord_init(void);
...
/* MODEL data handler : MODEL データハンドラ */
...
void model(GsOT *wot, int otlen)
{
	...
	for (j=0; j<PARTNUM; j++){
	    ...
	    /* handling parts are lit up : 操作するオブジェクトは明るく表示 */
...
/* initialize model data モデリングデータ初期化 */
void model_coord_init(void)
{			
    ...
    oppp = PacketArea;	/* preset packet area: packetの枠組を作るアドレス */

    
    for (i=0; i<DISPNUM; i++){
	for (j=0; j<PARTNUM; j++){
	    mhand= &model_hand[i][j];
	    /* initialize number of Objects : 論理オブジェクト総数初期化 */
	    mhand->Objnum=0;
	    /* set to lead fo object handler : オブジェクトハンドラの先頭へ */
	    objp=mhand->object;
	    if (parent_coord[j]<0){
...
static int sub_model_init(u_long *addr, u_long **opppp, GsDOBJ5 **objpp, GsCOORDINATE2 *co, u_long *on)
{
    ...
    *on += objntmp;
    ...
    for (i=0;i<objntmp;i++){
	/* default object coordinate : オブジェクトの座標系の設定 */
	(*objpp)->coord2 = co;
...
int model_move(u_long padd)
{
    ...
    static long mimepr[MIMENUM];	/* MIMe parameters: MIMe パラメータ */
    ...
    /* next preset angle set: 次のプリセット状態へ */
...
#define RT 10
    ...
    /* choose interpolation method menu : 内挿メニュー選択 */
    if (padd & A_PADstart & ~oldpadd){
	intrmenunum=(intrmenunum+1)%INTRNUM;
	intrm= &intrmenu[intrmenunum];
	intrm->start_func();
    }
    /* interpolation by {Euler angle or axes} MIMe */
    intrm->reset_func();
...
/***********************************************
 *	regularize angle (0-4095):  回転角の正規化 (0-4095)
 ***********************************************/
void marume(SVECTOR *sv)
{
...
/*    tmp1   = GsIDMATRIX;		/* 単位行列から出発する */
    ...
    /* flush the matrix cache : マトリックスキャッシュをフラッシュする */
    coor->flg = 0;
}
