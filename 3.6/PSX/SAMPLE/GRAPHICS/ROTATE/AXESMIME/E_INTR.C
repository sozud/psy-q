...
/* Euler Angle MIMe calc: 実際のオイラー角 MIMe 演算*/
void euler_interpolate(long *pr)
{
    int mime,j;
    SVECTOR *isv, *dsv;
    long *iv, *dv;
...
/* reset coordinates : 座標系をもとの状態 (base) へ戻す */
void euler_interpolate_reset(void)
{
    int i;
    long *bv, *iv;
