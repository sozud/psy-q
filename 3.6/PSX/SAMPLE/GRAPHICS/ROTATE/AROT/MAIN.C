...
{
  ResetCallback();
  ResetGraph(0);		/* GPU reset: GPUリセット */
  PadInit(0);			/* initialize pad: コントローラ初期化 */
  padd=0;			/* init pad value: コントローラ値初期化 */
  
  GsInitGraph(640,480,2,0,0);	/* interlace: （インターレースモード） */
  GsDefDispBuff(0,0,0,0);	/* double buffer: ダブルバッファ指定 */

/*  GsInitGraph(640,240,0,0,0);	/* non-interlace:（ノンインターレースモード）*/
/*  GsDefDispBuff(0,0,0,240);	/* double buffer: ダブルバッファ指定 */

  GsInit3D();			/* init 3D system: ３Dシステム初期化 */
  
  Wot[0].length=OT_LENGTH;	/* set OT length: OTハンドラに解像度設定 */
  Wot[0].org=zsorttable[0];	/* set OT: OTハンドラにOTの実体設定 */
  ...
  model_init();
  view_init();			/* set viewpoint: 視点設定 */
  light_init();			/* set light source; 平行光源設定 */
...
view_init()			/* set view point: 視点設定 */
{
  ...
  /* twist:  視点の捻りパラメータ設定 */
  view.rz=0;
  view.super = WORLD;		/* set view param: 視点座標パラメータ設定 */
...
light_init()			/* lighting: 平行光源設定 */
{
  /* Light 0: ライトID０ 設定 */
  /* lighting direction: 平行光源方向パラメータ設定 */
  pslt[0].vx = 100; pslt[0].vy= 100; pslt[0].vz= 100;
  /* lighting color: 平行光源色パラメータ設定 */
  pslt[0].r=0xd0; pslt[0].g=0xd0; pslt[0].b=0xd0;
  /* set light param.: 光源パラメータから光源設定 */
  ...
  /* set lighting mode: 光源計算のデフォルトの方式設定 */
  GsSetLightMode(0);
}

coord_init()			/* coordinates: 座標系設定 */
{
  /* DCube directly connected to World: 座標系はワールドに直にぶら下がる */
  GsInitCoordinate2(WORLD,&DCube);
  PWorld.vx=PWorld.vy=PWorld.vz=0;

  PCube = PWorld;

  /* Dcube origin to World(0,0,0): Cubeの原点をワールド座標系の(0,0,0)に設定 */
  DCube.coord.t[0] = 0;
  DCube.coord.t[1] = 0;
  DCube.coord.t[2] = 0;
}
...
set_coordinate(pos,coor)        /* マトリックス計算ワークからマトリックス
                                   を作成し座標系にセットする */
SVECTOR *pos;                   /* matrix work: マトリックス計算ワーク */
GsCOORDINATE2 *coor;            /* coordinate: 座標系 */
{
  MATRIX tmp1;
  SVECTOR v1;

  tmp1   = GsIDMATRIX;          /* unit matrix: 単位行列から出発する */
  				/* set transfer: 平行移動をセットする */
