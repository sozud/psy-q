...
typedef struct demo {
  ...
  char *fn;			/* bin-file name :: BINファイル名 */
  int (*func)();		/* function name :: 呼び出す関数 */
} DMENU;
...
#define SEQ_SIZE 12738		/* stay in parent process :: 
					親プロセス側に常駐 */
#define VH_SIZE 3104		/* stay in parent process ::
					親プロセス側に常駐 */
...
main()
{
  ...
  while(1){
	...
	Puts(MENU_X,y,dm[j].s);
      ...
      if (pad & PAD_A_START){
	file_read(dm[i].fn,LoadAddress);/* read child :: 子プロセスロード */
	dm[i].func();		/* run child :: 子プロセス実行 */
	break;
      }
      if (pad & PAD_A_UP)
	i--;
...
void *address;
{
  ...
  for (i = 0; i < 10; i++) {	/* 10 times retry :: リトライは 10 回 */
    ...
    cnt = _read1(fp.size,address,mode); /* read :: リード */

    /* if it success, break. :: 正常終了ならばブレーク */
    if (cnt == 0)
      return(0);		/* success :: 成功 */
  }
