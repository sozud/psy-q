...
SVECTOR RotV[OBJMAX];			/* rotation speed(angular velocity) : 回転スピード（角速度） */
...
#define DISTANCE 600			/* initial value for Radial : Radialの初期値 */
...
main()
{
	...
	PadInit(0);
                   ...
                   CDREADを定義していない時は、あらかじめPQBLOADで
			       データをダウンロードしておいて下さい */
...
/*
 *  draw 3D object (cube) : 3Dオブジェクト（キューブ）の描画
 */
void drawCubes()
{
	int i;
	...
	/* enter 3D object (cube) into OT : 3Dオブジェクト（キューブ）のOTへの登録 */
	objp = object;
	for(i = 0; i < nobj; i++) {
		...
		/* translation capacity -> set in matrix : 平行移動量->マトリクスにセット */
	...
	/* include pad data in buffer : パッドの内容をバッファに取り込む */
	oldpad = PadData;
	PadData = PadRead(1);
	...
	/* forcibly stop drawing operation for previous frame : 前のフレームの描画作業を強制終了 */
	ResetGraph(1);

	/* swaap double buffer : ダブルバッファを入れ換える */
	GsSwapDispBuff();
...
{	
	...
	for(i = 0; i < NCUBE; i++) {
		...
		/* set initial position (read from pos.h) : 初期位置の設定(pos.hから読む) */
		Trns[i].vx = *posp++;
		Trns[i].vy = *posp++;
		Trns[i].vz = *posp++;
		Rot[i].vx = 0;
...
u_long *tmdp;
{
	...
	tmdp++;
	tmdobjnum = *tmdp;
	tmdp++; 		/* point to object at beginning : 先頭のオブジェクトをポイント */
	TmdBase = tmdp;
}

/*
 *  read in texture(transfer to VRAM) : テクスチャの読み込み（VRAMへの転送）
 */
...
void initView()
{
	...
	/* parent coordinates of viewpoints : 視点の親座標 */
	View.super = WORLD;
	...
	/* mode = normal lighting : モード = 'normal lighting' */
	GsSetLightMode(0);
}
...
int factor;	/* brightness factor (0-255) : 明るさのファクター(0～255) */
{
	if(c_mode == 0) {
		...
		/* cocktail lights (using Gouraud) : カクテルライト（Gouraudで使用） */
...
void changeTmd()
{
	...
	/* switch color/brightness of light source according to 
	   TMD type : TMDの種類にあわせて光源の色/明るさを切り替え */
	switch(CurrentTmd) {
	    case 0:
