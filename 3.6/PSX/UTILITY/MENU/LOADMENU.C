...
int load_menu(MENU **menu, int *menu_max)
{
    ...
    *menu_max = 0;
    ...
    /* menu item number : アイテム数 */
    *menu_max = 200;

    /* allocate of menu informations : メニュー情報領域をアロケート */
    if(alloc_menu(menu, menu_max) == -1) {
        *menu_max = 0;
        return -1;
    }

    /* read menu : メニューをアロケートしたバッファに一括して読む */
#ifndef CDEXEC
    ...
    /* get item information : メニュー情報を取得 */
    *menu_max = get_menu(buf, menu, fixroot);

    /* reset file name: ファイル名の再設定 */
...
static void reset_root(MENU *menu,int *menu_max,char *fixroot)
{
    ...
    for ( mp = menu, mc = 0; mc < *menu_max; ) {
        ...
        /* for dname: dname の頭に root を cat する */
        ...
        /* get ROOT: root を fixroot に合わせて変更する */
        strcpy(buf, mp->root);
        ...
        /* make path for fname: fname のパスを作る */
        for ( depth = 0; depth < MAX_DIRDEPTH && str; depth++ ) {

	    /* パスをディレクトリごとに分解 */
            if ( (str = strchr(buf,'\\')) ) {
                /* set dir name: ディレクトリ名 */
