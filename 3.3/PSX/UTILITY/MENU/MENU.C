 ...
 * Primitive Buffer
 */
#define OTSIZE		8		/* ordering table size: OT の大きさ */
typedef struct {
	DRAWENV		draw;		/* drawing environment: 描画環境 */
	DISPENV		disp;		/* display environment: 表示環境 */
	u_long		ot[OTSIZE];	/* ordering table: OT */
...
main()
{
	...
	DB	*cdb;		/* current double buffer
				 : ダブルバッファのうちの現在のポインタ */
	int	depth=0;	/* the depth of tree : 階層 */
	int	wait=0;		/* work */
	int	newdepth;	/* work */
	FLAGS	fl={0,0,0,0};	/* flags */
	...
	/* initialize graphics system: グラフィックシステムの初期化 */
	init_system(db);
	...
	*/
	...
	/* clear the dm: dm を ０クリア */
	memset(dm,0,sizeof(DISP_MENU)*MAX_DIRDEPTH);

	/* load menu file: メニューファイルを読む */
	load_menu(&menu, &menu_max);

	/* select items to display: 画面に表示するアイテムを選ぶ */
	set_dispitem(depth,menu,menu_max,&dm[depth]);

	/* initialize sound: サウンドの初期化 */
	sndInit();
	...
	while (1) {
		...
		/* swap double buffer ID: ダブルバッファID の切替え */
		cdb = (cdb==db)? db+1: db;

		/* clear ordering table: OT のクリア */
		...
		/* wait for end of drawing: 描画の終了を待つ */
		DrawSync(0);

		/* wait for V-BLNK (1/60): 垂直同期を待つ（1/60秒） */
		VSync(0);

	}
}

/* initialize graphic system: グラフィックシステムの初期化 */
static void init_system(DB *db)
{
	PadInit(0);		/* reset PAD */
...
static void init_prim(DB *db)
{
	...
	/* initialize POLY_G4: POLY_G4 の初期化 */
	sp = &db->poly;
...
/* Read controll-pad: コントローラデータの読み込み */
static int pad_read(MENU *menu,int depth,DISP_MENU *dm,int *execno,FLAGS *fl)
{
	MENU		*mp;
	...
	/* Change Languge: 言語の切替え */
	if (padd & PADselect)
		fl->lang ^= 1;

	/* MENU or HELP mode: ヘルプモードへの切替え */
	if (padd & PADRleft)
		fl->help = 1;
