...
main()
{
	...
	total_time = CdPosToInt(&loc[0]);
	CdIntToPos(total_time, &loc[99]);
	...
	while (((padd = PadRead(1))&PADselect) == 0) {
		...
		FntPrint("playing (%02x:%02x:%02x)...%d\n",
			 pos.minute, pos.second, pos.sector, next_v-VSync(-1));
