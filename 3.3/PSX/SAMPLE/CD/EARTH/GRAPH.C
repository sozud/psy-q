...
#define MODELADDR	((u_long *)0x80210000)	/* TMD のアドレス */
...
init_graph()
{
	...
	db_init(320, 240, SCR_Z);	/* ダブルバッファの初期化 */
...
graph()
{
	...
	if ((nobj = pad_read(obj, nobj, &world)) == -1)
		return(-1);
...
static int pad_read(OBJ_FT3 *obj, int nobj, MATRIX *m)
{
	...
	if (padd & PADi)	vec.vz += 16;
	if (padd & PADj) 	vec.vz -= 16;
	if (padd & PADl)	scale  += ONE/32;
	if (padd & PADn)	scale  -= ONE/32;
