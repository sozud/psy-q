...
#define SCR_Z	512			/* distance to screen */
#define OTSIZE	(SCR_Z>>2)		/* ordering table size */
...
#define PT_TX	(640/PT_NX)		/* cell size (in XY space) */
#define PT_TY	(240/PT_NY)
...
{
	mesh_init(&Mesh[0][0], PT_NX+1, PT_NY+1, 640, 240);
...
#define DANG	64		/* rotate speed */
#define DMORF	16		/* morphing speed */
#define DSCRL	8		/* scroll speed */
...
static pad_read(void)
{
	...
	if (padd & (PADl|PADn)) {	/* move morphing position */
		if (padd & PADRup)	Mpos[1]--;
		if (padd & PADRdown)	Mpos[1]++;
		if (padd & PADRleft) 	Mpos[0]--;
		if (padd & PADRright)	Mpos[0]++;
		...
		limitRange(Srate[0], -80, 80);
		limitRange(Srate[1], -80, 80);
	}
	
	/* update morphing depth */
	if (padd & PADl)	Mheight -= DMORF;
	if (padd & PADn)	Mheight += DMORF;
