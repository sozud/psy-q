...
#include <libgte.h>
#include <libgpu.h>
#include <libgs.h>	/* グラフィックライブラリ を使うための 構造体などが
			   定義されている */

#define OBJECTMAX 100	/* ３Dのモデルは論理的なオブジェクトに分けられる
			   この最大数 を定義する */
...
unsigned long addr;
{
  ...
  GsGetTimInfo((unsigned long *)(addr+4),&tim1); /* TIMデータのヘッダからテクスチャの
					     データタイプの情報を得る */
...
{				/* モデリングデータの読み込み */
  ...
  dop=(unsigned long *)MODEL_ADDR;	/* モデリングデータが格納されているアドレス */
  dop++;			/* モデリングデータ（TMDフォーマット）
				   hedder skip */
