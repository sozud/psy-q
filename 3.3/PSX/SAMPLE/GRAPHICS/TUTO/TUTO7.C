...
main()
{
	...
	/* world/local matrix: ワールド／ローカル マトリックス */
	MATRIX		world, local;	
	
	/* light source matrix: 光源の回転マトリックス */
	MATRIX		rotlgt;		
	...
	while (pad_read(&ncube, &world, &local, &rotlgt) == 0) {
		...
		/* add cubes to the OT: ncube 個の立方体を登録 */
		for (i = 0; i < ncube; i++) {
			RotTrans(&attr[i].trans, (VECTOR *)local.t, &flg);
			add_cubeF4L(cdb->ot, cdb->c[i].s, &local, attr[i].col);
		}
...
/* MATRIX *world, 	rottrans matrix for all: 全体の回転マトリックス */
/* MATRIX *local, 	rottrans matrix for each: 各立方体の回転マトリックス */
/* MATRIX *rotlgt	light source matrix: 光源の回転マトリックス */
static int pad_read(int *ncube, 
		MATRIX *world, MATRIX *local, MATRIX *rotlgt)
{
	 ...
	 */
	MulMatrix2(world, local); 
	
	/*: make light matrix:  光源の回転マトリックスの計算 */
	RotMatrix(&lgtang, rotlgt);
	MulMatrix(rotlgt, local);

	/* scale of the local matrix represents the scale of the local object.
	 : オブジェクトのスケールはローカルマトリクスのスケールで表現できる */
	ScaleMatrix(local, &scale);
	
	/*Setting Matrix for all the objects）
	  : マトリックスの設定（全体） */
	SetRotMatrix(world);	/* rotation: 回転マトリックス */
	SetTransMatrix(world);	/* translation: 平行移動ベクトル */
